...
// 2. GET http://localhost:8000/api/employees?page=1 Devuelve los Empleados del 0 al 1 del listado.
// Se utiliza el método de JavaScript slice, solución simplificada.
app.get("/api/employees?page=1", (req, res) => {
  const startIndex = 0;
  const endIndex = 1;
...
// 2. GET Devuelve los Empleados 2 y 3 del listado. http://localhost:8000/api/employees?page=2
// Se utiliza el método de JavaScript slice, solución más simplificada.
app.get("/api/employees?page=2", (req, res) => {
  const startIndex = 2;
  const endIndex = 3;
